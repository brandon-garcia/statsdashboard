...
final class FormPanel extends \html\Panel{
    ...
    public function file($name,$label,$required=false) {
        $this->setNodeId();
        $this->inputs[$this->node_id] = $this->inputBlock($name,$label,
            "<input class=\"input-file\"
                id=\"$name\" name=\"$name\"  type=\"file\"".($required?' required':'').">");
        return $this;
    }
    ...
    public function textarea($name,$label,$required=false) {
        ...
        $this->inputs[$this->node_id] = $this->inputBlock($name,$label,
            "<textarea class=\"form-control\"
                id=\"$name\" name=\"$name\"".($required?' required':'')."></textarea>");
        return $this;
    }
    ...
    public function textInput($name,$label,$required=false) {
        ...
        $this->inputs[$this->node_id] = $this->inputBlock($name,$label,
            "<input type='text' class=\"form-control\"
                id=\"$name\" name=\"$name\"".($required?' required':'')."></input>");
        return $this;
    }

    public function numberInput($name,$label,$minval,$required=false) {
        $this->setNodeId();
        $this->inputs[$this->node_id] = $this->inputBlock($name, $label,
