...
require_once __DIR__ . '/../../SQL/sql_result.php';

function format_val($type,$val){
	...
	public function __construct($sql_result){
		...
		for($irec=0; $irec<count($this->myRecords); ++$irec)
			for($ival=0; $ival<count($this->myRecords[$irec]); ++$ival)
				$this->myRecords[$irec][$ival] = format_val($this->myFields[$ival]['type'],$this->myRecords[$irec][$ival]);
	}
